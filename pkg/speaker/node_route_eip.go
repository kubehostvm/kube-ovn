...
func (c *Controller) enqueueAddNodeRouteEIP(obj any) {
	...
	if !eip.Status.Ready {
		klog.V(3).Infof("skip enqueueing non-ready iptables-eip %s on add", eip.Name)
		return
	}
...
// When an EIP is deleted, we need to withdraw its BGP announcement immediately.
func (c *Controller) enqueueDeleteNodeRouteEIP(obj any) {
	...
	// For delete events, withdraw the route immediately without going through the queue
...
func (c *Controller) withdrawEIPRoutes(eip *kubeovnv1.IptablesEIP) {
	if eip.Spec.V4ip == "" && eip.Spec.V6ip == "" {
		return
	}

	klog.V(3).Infof("withdrawing BGP routes for iptables-eip %s (V4ip=%s, V6ip=%s)",
		eip.Name, eip.Spec.V4ip, eip.Spec.V6ip)

	var errs []error
	if eip.Spec.V4ip != "" {
		if err := c.delRoute(eip.Spec.V4ip); err != nil {
			klog.Errorf("failed to withdraw BGP route for EIP IPv4 %s: %v", eip.Spec.V4ip, err)
			errs = append(errs, err)
		}
	}
	if eip.Spec.V6ip != "" {
		if err := c.delRoute(eip.Spec.V6ip); err != nil {
			klog.Errorf("failed to withdraw BGP route for EIP IPv6 %s: %v", eip.Spec.V6ip, err)
			errs = append(errs, err)
		}
	}
	if len(errs) > 0 {
...
func (c *Controller) handleAddOrUpdateNodeRouteEIP(eipName string) error {
	...
	// Only announce ready EIPs; withdraw routes if EIP becomes non-ready
	if !eip.Status.Ready {
		klog.V(3).Infof("iptables-eip %s not ready, withdrawing routes", eipName)
		c.withdrawEIPRoutes(eip)
		return nil
	}
	...
	if !c.hasNatGwPodOnLocalNode(eip) {
		klog.V(3).Infof("NAT GW pod for iptables-eip %s not on local node %s, skipping",
	...
	klog.Infof("announcing BGP routes for iptables-eip %s (V4ip=%s, V6ip=%s)",
		eipName, eip.Spec.V4ip, eip.Spec.V6ip)

	var errs []error
	if eip.Spec.V4ip != "" {
		if err := c.addRoute(eip.Spec.V4ip); err != nil {
			klog.Errorf("failed to announce BGP route for EIP IPv4 %s: %v", eip.Spec.V4ip, err)
			errs = append(errs, err)
		}
	}
	if eip.Spec.V6ip != "" {
		if err := c.addRoute(eip.Spec.V6ip); err != nil {
			klog.Errorf("failed to announce BGP route for EIP IPv6 %s: %v", eip.Spec.V6ip, err)
			errs = append(errs, err)
		}
	}
